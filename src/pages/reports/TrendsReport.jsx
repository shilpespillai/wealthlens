...
import { 
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Line, ComposedChart, Cell, AreaChart, Area, ReferenceLine
} from "recharts";
...
const MOCK_TRANS_GEN = (() => {
  ...
  const transactions = [];
  months.forEach((m) => {
    ["Salary and Wages", "Household", "Food", "Entertainment", "Fuel / Gas", "Healthcare"].forEach(cat => {
      const amount = (cat === "Salary and Wages") ? 5500 : (CATEGORY_BUDGETS[cat] * (0.8 + Math.random() * 0.5));
      transactions.push({
        id: `mock-${m}-${cat}`,
        date: `${m} 15 2026`,
        merchant: cat,
        amount: (cat === "Salary and Wages") ? amount : -amount,
        category: cat,
        type: (cat === "Salary and Wages") ? 'income' : 'expense'
      });
    });
  });
  return transactions;
...
export default function TrendsReport() {
  ...
  const chartData = useMemo(() => {
    return activeInterval.map((month, idx) => {
      ...
      if (selectedCategory === "All categories") {
        ...
        budgeted = showType === 'income' ? 5500 : 2800; // Match screenshot budget line peaks
      } else {
        actual = monthTxs
  ...
  const burndownData = useMemo(() => {
    ...
    const budget = CATEGORY_BUDGETS[selectedCategory] || (showType === 'income' ? 5500 : 2800);
    ...
    return days.map((day, idx) => {
      // Calculate spend for this specific day
  ...
  }, [allTransactions, selectedCategory, dateRange, showType]);
        ...
        <main className="flex-1 overflow-y-auto p-12 pt-6 bg-[#F8F9FB]">
           <div className="max-w-6xl mx-auto space-y-8">
              ...
              <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
                 {/* Large Chart Card */}
                 <div className="xl:col-span-3 bg-white border border-slate-200 rounded-[40px] p-12 shadow-sm relative group hover:shadow-xl hover:shadow-slate-200/40 transition-all duration-500">
                 ...
                 <div className="xl:col-span-1 space-y-8 h-full">
                    <div className="bg-white border border-slate-200 rounded-[40px] p-10 shadow-sm h-full flex flex-col justify-between hover:shadow-xl transition-all duration-300">
                       <div>
